...
BEGIN {
	...
	sigev_count = 0
	si_count = 0
	ill_count = fpe_count = segv_count = bus_count = trap_count = cld_count = poll_count = 0
}
...
/^#define[ \t]+[A-Z_][A-Z0-9_]+[ \t]+/ {
	...
	# Skip macro expressions (but tolerate bitwise operations for BSD SA_*)
	if (val ~ /[(+*/]/) next
...
END {
	# If this OS has no SIGRTMIN/SIGRTMAX, synthesize a friendly pair
	if (!("SIGRTMIN" in sigs) || !("SIGRTMAX" in sigs)) {
		maxsig = 0
		for (s in sigs) {
			v = sigs[s] + 0
		...
		if (!("SIGRTMAX" in sigs)) {
			sigs["SIGRTMAX"] = maxsig + 1
			sigs_count++
		}
		if (!("SIGRTMIN" in sigs)) {
			sigs["SIGRTMIN"] = maxsig + 2
			sigs_count++
		}
	}

	# Synthesize core POSIX SIGEV_* if missing
	if (sigev_count == 0) {
		sigev["SIGEV_NONE"]   = 0
		sigev["SIGEV_SIGNAL"] = 1
		sigev["SIGEV_THREAD"] = 2
		sigev_count = 3
	}
